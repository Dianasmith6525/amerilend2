...
export async function checkCryptoPaymentStatus(
  ...
  // In production, call API to check status
  /*
  const config = getCryptoPaymentConfig();
  ...
  return {
    status: latestEvent.status === "COMPLETED" ? "confirmed" : "pending",
    txHash: data.data.payments?.[0]?.transaction_id,
    confirmations: data.data.payments?.[0]?.block?.confirmations,
  };
  */
